...
class FuzzyCTCLossOp : public OpKernel {
 ...
 public:
  explicit FuzzyCTCLossOp(OpKernelConstruction* ctx) : OpKernel(ctx) {
  ...
  void Compute(OpKernelContext* ctx) override {
    ...
    ctc::FuzzyCTCLossCalculator::LabelSequences labels_t(batch_size);
    ...
    for (std::size_t b = 0; b < batch_size; ++b) {
      input_list_t.emplace_back(inputs_t.data() + b * max_time * num_classes,
                                max_time, num_classes);
      gradient_list_t.emplace_back(
          gradient_t.data() + b * max_time * num_classes, max_time,
          num_classes);
    }
    ...
    ctc::FuzzyCTCLossCalculator ctc_loss_calculator(num_classes - 1);
 ...
 private:
  ...
  TF_DISALLOW_COPY_AND_ASSIGN(FuzzyCTCLossOp);
};

REGISTER_KERNEL_BUILDER(Name("FuzzyCTCLoss").Device(DEVICE_CPU), FuzzyCTCLossOp);
