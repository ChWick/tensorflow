...
class FuzzyCTCDecoder {
 public:
  ...
  FuzzyCTCDecoder(int num_classes, int batch_size, bool merge_repeated)
      : num_classes_(num_classes),
        blank_index_(num_classes - 1),
        batch_size_(batch_size),
        merge_repeated_(merge_repeated) {}

  virtual ~FuzzyCTCDecoder() {}
...
class FuzzyCTCGreedyDecoder : public FuzzyCTCDecoder {
 public:
  FuzzyCTCGreedyDecoder(int num_classes, int batch_size, bool merge_repeated)
      : FuzzyCTCDecoder(num_classes, batch_size, merge_repeated) {}

  Status Decode(const FuzzyCTCDecoder::SequenceLength& seq_len,
                const std::vector<FuzzyCTCDecoder::Input>& input,
                std::vector<FuzzyCTCDecoder::Output>* output,
                FuzzyCTCDecoder::ScoreOutput* scores) override {
    if (output->empty() || (*output).size() < batch_size_) {
